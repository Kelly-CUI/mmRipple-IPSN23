...
%   -----------------------------------------------------------------------
%   Description:    This function loads bin data and perform data parsing, outputs radarCube
%   Input:          setupJsonFileName - mmWave radar configuration
%                   rawDataFileName - bin data
%                   path - bin data path
%
%   Output:         radarCube
%                       radarCube.rfParams- rf parameters(startFreq,Slope,BW...);
%                       radarCube.dim - data dimension(#freame, #chirp, #Rx)
%                       radarCube.data - chirp data after range FFT(complex)  * #Rx * # chirps for one frame   
%   -----------------------------------------------------------------------
%
%
function readerRawData(setupJsonFileName, rawDataFileName, path)
    close all;
    ...
    EXIT_KEY_PRESSED = 0;
    radarCubeDataFileName = 'radarCube'; 
    ...
    for idx=1:numBinFiles
        binFileName{idx} = strcat(binFilePath, '\', rawDataFileName);
    end
    
    % Save path
     savePath=[binFilePath,'\matData\'];
     mkdir(savePath); 
    ...
    % Export data
    dp_exportData(rawDataFileName, radarCubeDataFileName,savePath);
...
%   Output:         mat files
%   -----------------------------------------------------------------------
function dp_exportData(rawDataFileName, radarCubeDataFileName, savePath)
    global dataSet
    global Params
...
%         
%         saveData = [savePath, rawDataFileName(1:end-4)];
%         
%         % Save params and data to mat file        
% %         save (saveData, 'adcRawData', '-v7.3');
%     end
    ...
    if (~strcmp(rawDataFileName, ''))
        ...
        radarCube.data_rangeFFT = radarCubeData;
        ...
        radarCube.rfParams.rangeFFT_x = [0:1:radarCube.rfParams.numRangeBins-1].*radarCube.rfParams.rangeResolutionsInMeters;

        saveData = [savePath, rawDataFileName(1:end-4)];
        
        % Save params and data to mat file
        %save (saveData,'adcRawData','radarCube', '-v7.3');
         save (saveData,'radarCube', '-v7.3');
    end
end
%   -----------------------------------------------------------------------
%   Description:    This function loads one frame data and perform range
...
function [radarCubeData] = processingChain_rangeFFT(rangeWinType) 
    ...
    WIN = repmat(win',NChirp,1);
    
    %radarCubeData = single(zeros(NChirp,NChan,NRangeBin));
    radarCubeData = single(zeros(NChirp,NRangeBin,NChan));
    for chIdx = 1: Params.NChan
            temp = dataSet.rawFrameData(:,chIdx,:);
            temp = reshape(temp,NChirp,NRangeBin);
            outfft = fft(temp .* WIN, NRangeBin,2); 
            radarCubeData(:,:,chIdx) = outfft;
     end
